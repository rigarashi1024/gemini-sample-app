...
2. **AI によるアンケート自動生成（汎用）**
   - 目的を解析し、回答者から収集すべき情報を AI が判断し、
     ...
     5. Yes/No の二択問題は明確に Yes / No として生成する  
        - 「たぶん」「どちらともいえない」など曖昧な選択肢を追加する場合は、意味が明確になるように名称を統一する。
     6. AI は必ず Questions JSON の仕様に完全準拠した出力を行う  
     ...
     8. free_text が必要な質問を作成する場合  
        - その質問は「集計対象外」として扱う前提で作成し、free_text 問題を多用しないこと。
     9. 質問生成時、ユーザー入力に明示的な地名が含まれない限り、AI は特定の地域名（例：渋谷、新宿、梅田など）を質問文に挿入してはならない。
     10. 「場所」「エリア」に関する項目を作成する場合は、原則として text（自由入力）または大分類の選択肢 + 自由入力を使用し、具体的な地名を固定した選択肢を生成しないこと。
...
- AI生成したアンケート内容（Questions）
  ...
  - AI生成されたアンケートとは別に `deadline` を指定できる質問を任意項目として最後に追加してください
  - 選択形式のアンケートについては選択肢を編集できるようにする
    - 自動生成されたものは削除できるようにする
    - 最終行に新しいアンケートを手動で追加できるようにする
    - アンケートの選択肢の順番をドラッグ＆ドロップで入れ替えることができるようにする  
- 「アンケート再生成」ボタン
...
- アンケート編集時にはヘッダーに「前のページに戻る」ボタンを用意する
  - 「前のページに戻る」ボタンを用意するクリック時は遷移前のページに戻る
- 送信ボタン
...
- PRでの確認をメインで行うため、削除を伴わないコード変更によるUserへの確認は不要です

### 7. ファイル・ディレクトリ削除ルール

- プロジェクト内ファイルの削除は許可されている
- ただし以下を必ず守ること

#### 削除時の必須ルール

1. 削除を行った場合、必ず PR の説明に以下を記載する
   - 削除したファイル / ディレクトリ一覧
   - 削除理由には、以下のいずれかを必ず含めること：
     - どこからも参照されていない（検索条件）
     - 代替実装がどこにあるか
     - 今後も使われる予定がない理由
   - 影響範囲（ルーティング / API / テスト / UI など）
...
3. 迷いがある場合は削除せず、コメントで「削除候補」として列挙する

4. 削除を行う場合は、必ず「削除のみを含む単独コミット」にまとめること  
   - リファクタリングや移動・修正と混在させてはならない  
   - コミットメッセージ例:  
