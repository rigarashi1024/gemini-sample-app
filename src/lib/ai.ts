...
4. Yes/Noの二択問題は明確にYes/Noとして生成する
   - 「たぶん」「どちらともいえない」など曖昧な選択肢を追加する場合は、意味が明確になるように名称を統一する

5. Questions JSONの仕様に完全準拠した出力を行う
...
7. text型が必要な質問を作成する場合
   - その質問は「集計対象外」として扱う前提で作成し、text型の質問を多用しないこと

8. 質問生成時、ユーザー入力に明示的な地名が含まれない限り、特定の地域名（例：渋谷、新宿、梅田など）を質問文に挿入してはならない

9. 「場所」「エリア」に関する項目を作成する場合は、原則としてtext（自由入力）または大分類の選択肢+自由入力を使用し、具体的な地名を固定した選択肢を生成しないこと
